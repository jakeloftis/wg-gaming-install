...
# Ubuntu 22.04 VPS
# =============================================================================
# Forwards ports 1024-65535 TCP+UDP to player1
# Compatible with all games, not just Call of Duty
#
# USAGE:
...
echo -e "  Forwarded ports  : ${GREEN}${GAME_PORT_START}-${GAME_PORT_END} TCP+UDP${NC}"
echo -e "${YELLOW}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
echo ""
...
# ── Step 7: Write complete wg0.conf ───────────────────────────────────────────
info "Writing wg0.conf with universal port forwarding (${GAME_PORT_START}-${GAME_PORT_END})..."
...
# ── Universal port forwarding ${GAME_PORT_START}-${GAME_PORT_END} TCP+UDP ─────
PostUp = iptables -t nat -A PREROUTING -i ${PUB_IFACE} -p tcp --dport ${GAME_PORT_START}:${GAME_PORT_END} -j DNAT --to-dest ${PLAYER1_IP}
PostUp = iptables -t nat -A PREROUTING -i ${PUB_IFACE} -p udp --dport ${GAME_PORT_START}:${GAME_PORT_END} -j DNAT --to-dest ${PLAYER1_IP}

# ── Teardown ──────────────────────────────────────────────────────────────────
PostDown = iptables -t nat -D POSTROUTING -s ${WG_NET} -o ${PUB_IFACE} -j MASQUERADE
PostDown = iptables -D FORWARD -i wg0 -j ACCEPT
PostDown = iptables -D FORWARD -o wg0 -j ACCEPT
PostDown = iptables -t nat -D PREROUTING -i ${PUB_IFACE} -p tcp --dport ${GAME_PORT_START}:${GAME_PORT_END} -j DNAT --to-dest ${PLAYER1_IP}
PostDown = iptables -t nat -D PREROUTING -i ${PUB_IFACE} -p udp --dport ${GAME_PORT_START}:${GAME_PORT_END} -j DNAT --to-dest ${PLAYER1_IP}
...
if command -v ufw &>/dev/null && ufw status | grep -q "Status: active"; then
    ...
    ufw allow ${GAME_PORT_START}:${GAME_PORT_END}/tcp comment 'Gaming TCP' > /dev/null
    ufw allow ${GAME_PORT_START}:${GAME_PORT_END}/udp comment 'Gaming UDP' > /dev/null
    ufw reload > /dev/null
